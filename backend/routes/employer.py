...
def suggest_job_skills():
    ...
    try:
        model_names = ['models/gemini-2.5-pro', 'models/gemini-2.5-flash',
                       'models/gemini-pro-latest', "models/gemma-3-27b-it"]
        model = None
...
{{
  ...
  "complementary_skills": ["skill1", "skill2", ...]
        ...
        return jsonify({"msg": "Failed to suggest skills", "error": str(e)}), 500
...
def analyze_candidate_ats(job_id):
    ...
    try:
        ...
        score = int((len(matched) / len(required)) * 100) if len(required) > 0 else 0
        ...
        return jsonify({"msg": "Failed to analyze candidate", "error": str(e)}), 500


@employer_bp.route("/application/<job_id>/<candidate_id>/status", methods=["PUT"])
@jwt_required()
def update_application_status(job_id, candidate_id):
    ...
    if new_status not in ['accepted', 'rejected', 'pending', 'interviewing']:
    ...
    try:
        ...
        status_messages = {
            ...
            'rejected': f"Your application for {job.title} was not selected this time",
            'interviewing': f"You've been shortlisted for an interview for {job.title}",
            'pending': f"Your application for {job.title} is under review"
        }

        send_notification(
            candidate_id,
            status_messages.get(new_status, f"Application status updated for {job.title}"),
            type="application_status",
            link="/applications"
        )

        return jsonify({"msg": "Application status updated successfully", "status": new_status}), 200
